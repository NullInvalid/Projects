...
namespace PayrollSytem
{
    public partial class frm_Employees : Form
    {
        ...
        private void btnempsave_Click(object sender, EventArgs e)
        {
            ...
            if (txtcode.Text == "" || txtfname.Text== "" || txtmname.Text == "" || txtlname.Text == "" 
                || txtaddress.Text == "" || txtstatus.Text == "" || txtbplace.Text == "" 
                || txtage.Text == "" || txtemerg.Text == "" || txtdrate.Text == "" || cbPosition.Text == "" )
            {
                MessageBox.Show("Fill out the empty textboxes.", "Notification", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                if(rdomale.Checked)
                ...
                sql = "INSERT INTO `employee_workinfo` (`emp_code`, `d_rate`, `p_method`, `position`, `w_status`, `d_hired`)" 
                        + " VALUES ('" + txtcode.Text + "','" + txtdrate.Text + "','" + txtpmethod.Text + "','" + cbPosition.Text
                ...
                sql = "INSERT INTO `employee` (`emp_code`, `emp_fname`, `emp_lname`, `emp_mname`"
                   ...
                   + ", `emerg_contct`, `SSSNumber`) VALUES ('" + txtcode.Text + "','" + txtfname.Text + "','" + txtmname.Text
                   + "','" + txtlname.Text + "','" + txtaddress.Text + "'," + txtcontact.Text + ",'" + txtstatus.Text
                   + "','" + dtpdbirth.Text + "','" + txtbplace.Text + "','" + rdo + "'," + txtage.Text + ",'" + txtcontact.Text + ",'" + tb_SSS.Text + "')";
        ...
        private void txtcode_TextChanged(object sender, EventArgs e)
        {
            ...
            if(config.dt.Rows.Count > 0)
            {
                ...
                cbPosition.Text = config.dt.Rows[0].Field<string>("position").ToString();
...

        {
            ...
            else
            {
                ...
                sql = "UPDATE `employee_workinfo` SET  `d_rate`='" + txtdrate.Text 
                  + "', `p_method`='" + txtpmethod.Text + "', `position`='" + cbPosition.Text
                ...
                + "', `emp_mname`='" + txtmname.Text + "', `emp_lname`='" + txtlname.Text 
                ...
                + "', `emp_age`='" + txtage.Text + "', `emerg_contct`='" + txtcontact.Text + "', `SSSNumber`='" + tb_SSS.Text 
                + "' WHERE `emp_code`='" + txtcode.Text + "'";
                config.Execute_CUD(sql, "Error to execute.", "Data updated successfully.");
                //btnempnew_Click(sender, e);
        ...
        private void list_Employee()
        {
            sql = "SELECT `emp_code` AS 'CODE',CONCAT( `emp_fname`,' ', `emp_lname`,' ', `emp_mname`) AS 'NAME'" 
             ...
             + ", `contact` AS 'CONTACT', `SSSNumber` AS 'SSS'"+"  FROM `employee` WHERE `emp_code` LIKE '%" + txtempsearch.Text + "%'" 
        ...
        private void txtcontact_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
                {
                e.Handled = true;
                DialogResult da = MessageBox.Show("Numbers Only", "WARNING", MessageBoxButtons.OK);

            }

        }

        private void txtemerg_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
            {
                e.Handled = true;
        ...
        private void txtpmethod_SelectedIndexChanged(object sender, EventArgs e)
        {
